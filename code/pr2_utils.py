...
def match_stamps(T1: np.ndarray, T2: np.ndarray) -> defaultdict:
  return {ts: np.argmin(np.abs(ts - T2)) for ts in T1} 

def velocity(encoder_data: defaultdict, dt: np.ndarray) -> np.ndarray:
  wheel_radius = const.DDR.L / 2  # L/2
